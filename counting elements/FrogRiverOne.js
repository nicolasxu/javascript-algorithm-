...
The goal is to find the earliest time when the frog can jump to the
 ...
 at every position across the river from 1 to X. You may assume that 
 the speed of the current in the river is negligibly small, i.e. the 
...
function crossTo(leafSet, curPos) {
  // given current leaf status, and current position, 
  // return the new position.

  var newPos = curPos;

  while(leafSet[newPos+1]) {
    newPos++;
  }
  return newPos;
}
...
var inputA = [1, 3, 1, 4, 2, 3, 5, 4];
var X = 5;
var result = solution(X, inputA);
