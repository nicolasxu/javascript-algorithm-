...
peak. Notice in particular that the first block (1, 2, 3, 4) has a peak 
at A[3], because A[2] < A[3] > A[4], even though A[4] is in the adjacent block.
...
function solution(A) {
	...
	var left = peaks[0]; // index value
	var right = peaks[peaks.length -1]; // index value

	var segmentCount = peaks.length;

	while(segmentCount >=1) {
		
		var segLength = Math.floor(length / segmentCount);
		var peakIndex = 0;
		for (var segIndex = segLength - 1; segIndex < length; segIndex +=segLength) {

			if(Math.max(0, segIndex) <= peaks[peakIndex] && peaks[peakIndex])
			peakIndex++;
		}
...
var inputA = [1,2,3,4, 3,4,1,2, 3,4,6,2];
