...
the radiuses of the discs, is given. The J-th disc is drawn with 
...
// Algorithm is correct, 75% failed some performance test
function solution(A) {
	var length = A.length;
	var count = 0;
	for(var i = 0; i < length; i++) {
		for(var j = i+1; j < length; j++) {
			var aRadius = A[i];
			var aRightBoundary = i + aRadius;

			var bRadius = A[j];
			var bLeftBoundary = j - bRadius;

			if(aRightBoundary >= bLeftBoundary) {
				// intersect
				count++;
				if(count >=10000000) {
					return -1;
