...
integers is given. The consecutive elements of array A 
...
// algorithm is correct, but time out for performance cases
function solution_v1(A) {
...
//  70% score, 2 performance cases won't pass
function solution(A) {
  var headingEastCars = [];
  for(var i = 0; i < A.length; i++) {
    if(A[i] === 0) {
      // heading east
      headingEastCars.push(0);
    }
    if(A[i] === 1) {

      for(var j = 0; j < headingEastCars.length; j++) {
        headingEastCars[j]++;
      }
    }
  }
  var total = 0;
  headingEastCars.forEach(function(count){
    total = total + count;
  });
  return total;
}
