...
// 80%, failed some performance case
function solution(A) {

	var length = A.length;
	var minAvg;
	var minAvgStartIndex;
	var avgLength;

	for(var i = 0; i < length; i++) {	
		avgLength = 2;

		while(A[i + avgLength -1] >= A[i + avgLength -1 + 1]) {
			avgLength++;
		}
		// calculate the avg
		var total = 0;
		...
		var tempAvg = total/ avgLength;
		if(typeof minAvg === 'undefined') {
			minAvg = tempAvg;
			minAvgStartIndex = i;
		} else {
			if(tempAvg < minAvg) {
...
var inputA = [4,2,2,5,1,5,8];
var inputB = [5, 6, 3, 4, 9]; // 2
